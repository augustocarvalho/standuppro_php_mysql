<?php
  ...
  require_once "menu.php";
    ...
    if ( !empty($_GET['id'])) {
        $id = $_REQUEST['id'];
    	$sql = mysqli_query($con, "SELECT count(*) as filiado FROM filiacao WHERE atleta_cpf=$id and ano=2021");
        while ($row = mysqli_fetch_assoc($sql)){
         if ($row[filiado] == 0) { 
          ...
          while ($row_2 = mysqli_fetch_assoc($categoria)){
           if ( $row_2[categoria_idcategoria] <> 0 ) { 
              mysqli_query($con, "INSERT INTO filiacao (atleta_cpf, categoria_idcategoria, ano) 
                VALUES ('$id',  $row_2[categoria_idcategoria], 2021 )");
          ...
          mysqli_query($con, "DELETE FROM filiacao WHERE atleta_cpf = '$id' and ano = 2021");
