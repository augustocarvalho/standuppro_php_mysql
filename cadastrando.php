...
<table id="form_table">	
 <form method="post" action="?go=cadastro"> 
	...
	<tr>
        <td colspan="2"><input type="button" value="INÍCIO"  style="font-weight:bold" class="btn" onclick="location. href='./index.php' "> 
...
if (@$_GET['go'] == 'cadastro'){
  ...
  $estado = $_POST['estado'];
  $sql_result = mysql_query("select id_participante from participante where id_participante=$id_participante");
  if (mysql_num_rows($sql_result) == 0) {
    mysql_query("insert into participante (id_participante, nome_participante, estado, email) values('$id_participante','$nome','$estado','$email')");
